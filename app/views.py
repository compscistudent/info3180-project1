...
def userprofile(userid):
    ...
    if user:
        if request.headers.get('Content-Type') == 'application/json' or request.method == 'POST':
            ...
            return render_template('profile.html', fname = user.first_name , lname=user.last_name,created = user.created.strftime("%-d %b %Y"), username = user.username,userid=user.userid, gender = user.gender,age = user.age, img = user.image,bio=user.bio)
